...
gateway 起動中に `http://<gateway-host>:8766/editor` を開くと、同梱の
timeline editor で named motion template の下書きを作れます。エディタは
ブラウザ内だけで動作し、`avatar` / `leds` / `servo` トラックを持つ
draft `stackchan.motion_template.v0` schema の JSON ファイルを export します。
デバイスへの直接 push は、#89 で追跡している将来の
`register_motion(name, json)` runtime path に合わせて追加する想定です。
