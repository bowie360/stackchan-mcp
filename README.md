...
| `beat_mode_start(motion_intensity?, sensitivity?, color?, blink_rate?, motion_enabled?, led_enabled?, light_mode?, duration_sec?)` | Start gateway-only beat mode: continuously capture ambient device audio through the existing `listen` wire path, estimate BPM in the gateway, and flash the base ring. Head following is optional and off by default. `light_mode` supports `rainbow_flow` (default), `rainbow_pulse`, `soft_breathe`, and `solid_pulse`. While active, `listen()` is mutually exclusive; `say()` may interrupt and beat mode re-arms listening afterwards. Requires the `[stt]` extra for Opus decoding. | ✅ |
...
| `beat_mode_update(motion_intensity?, sensitivity?, color?, blink_rate?, motion_enabled?, led_enabled?, light_mode?)` | Update beat-mode VJ parameters without restarting capture. | ✅ |
